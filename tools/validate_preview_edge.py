...
checks = [
    ...
    ("clean video route includes compact optional SEO details and breadcrumbs", "buildVisibleSeo" in edge and "seo-video-copy" in edge and "seo-breadcrumbs" in edge and "video-details" in edge and "Video details" in edge),
    ("dynamic title is ID-unique", "const pageTitle = `${titleRaw} — ${id} | ${SITE_NAME}`" in edge),
    ("static watch generator includes compact optional SEO details", "video-details" in generator and "video-details-body" in generator and "Video details" in generator),
    ...
    ("edge accepts validated compact metadata fallback", "decodeVideoMeta" in edge and "__metaOnly" in edge and "encoded.length > 3600" in edge),
    ("meta-only canonical keeps exact preview payload", 'if (metaOnly) canonicalParams.set("meta"' in edge and "delete video.__metaOnly" in edge),
    ...
    ("preview image upstream fetch has timeout and size bounds", "fetchImageWithTimeout" in proxy and "AbortController" in proxy and "MAX_IMAGE_BYTES" in proxy and "readBytesWithLimit" in proxy),
    ("preview image unexpected errors are controlled", "preview-image failed safely" in proxy and 'onError: "bypass"' in proxy and "retry-after" in proxy),
