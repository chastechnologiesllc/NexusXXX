...
checks = [
    ...
    ("same-origin preview image function exists", proxy_path.exists() and 'path: "/preview-image"' in proxy and 'ImageResponse' in proxy and 'png-play-overlay-v3' in proxy),
    ...
    ("edge metadata uses versioned same-origin play-overlay URL", 'previewImageUrl' in edge and '/preview-image?url=' in edge and '&v=play3' in edge),
    ...
    ("dynamic fallback uses centered-play preview URL", '/preview-image?url=' in template and 'v=play3' in template and 'image/png' in template and 'content="640"' in template and 'content="480"' in template),
    ("preview image embeds a centered platform play button", 'borderWidth: "20px 0 20px 36px"' in proxy and 'width: 112' in proxy and 'height: 112' in proxy and 'top: 0' in proxy and 'left: 0' in proxy and 'alignItems: "center"' in proxy and 'justifyContent: "center"' in proxy),
    ...
    ("static generator uses versioned play-overlay PNG metadata", '&v=play3' in generator and 'image/png' in generator and 'content="640"' in generator and 'content="480"' in generator and 'video thumbnail with play button' in generator),
    ...
    ("share URL uses current preview cache version", 'params.set("nx_preview", "6")' in app),
    ("edge response carries current preview build marker", 'x-nexus-preview-version' in edge and 'share-play-overlay-6' in edge),
