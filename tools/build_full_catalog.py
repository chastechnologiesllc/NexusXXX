...
def build_outputs(db_path: Path, output_root: Path, ingest: dict[str, object]) -> dict[str, object]:
    ...
    featured_rows = conn.execute(
        """
        SELECT id,category_slug,category_name,title,slug,thumb,thumbFallback,duration,views,tags_json,embedSrc,source,added,pageUrl,
               ROW_NUMBER() OVER (PARTITION BY category_slug ORDER BY views DESC, id) - 1 AS catalog_index
        FROM videos
    ...
    for row in featured_rows:
        record = row_to_record(row[:-1])
        category_slug = str(row[1])
        catalog_index = int(row[-1])
        record["catalogFile"] = f"{category_slug}/part-{catalog_index // CHUNK_RECORDS + 1:04d}.json"
        record["catalogIndex"] = catalog_index % CHUNK_RECORDS
