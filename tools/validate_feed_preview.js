...
const checks = [
  ...
  ["native share uses share URL", app.includes('const shareData = { title: video.title, text: shareUrl, url: shareUrl }') && app.includes('navigator.share(shareData)')],
  ["runtime metadata uses play-overlay image URL", app.includes('crawlerPreviewImageUrl') && app.includes('preview-image-v2?url=') && app.includes('&v=play1')],
